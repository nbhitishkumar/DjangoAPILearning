# Generated by Django 3.1.3 on 2020-12-05 09:46

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('sellerMenu', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='SellerItem',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('item_name', models.CharField(blank=True, max_length=255, null=True)),
                ('item_description', models.CharField(blank=True, max_length=255, null=True)),
                ('item_image', models.CharField(blank=True, max_length=225, null=True)),
                ('item_preparing_time', models.TimeField(blank=True)),
                ('item_delivery_time', models.TimeField(blank=True)),
                ('item_price', models.IntegerField(default=0, null=True)),
                ('category', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, related_name='item_category_id', to='sellerMenu.sellercategory')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='user_item', to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'db_table': 'seller_item',
            },
        ),
    ]
